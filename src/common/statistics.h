...
class Statistics
{
	...
	std::map<int,int> m_pushedByThread; //! number of files queued for processing ( pushed ) by each thread map key = threadID, map value = number of files
	std::map<int, int> m_poppedByThread; //! number of files de-queued and processed ( popped ) by each thread  map key = threadID, map value = number of files
...
public:
	...
	*/
	...
	/**
	 * @brief new file pushed on thread's queue for processing
	 * @param threadID the id of the thread that will process file
	 */
